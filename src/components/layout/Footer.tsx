...
export default function Footer({
  logo,
  logoAlt = 'Moonochew Logo',
  siteName = 'Moonochew',
  tagline = "The world's only cattle-resistant, barbed stainless spring steel tree guard. Made in Australia, built to last forever.",
  contact,
  social,
}: FooterProps) {
  return (
      ...
      <div className="container-width py-16">
        <div className="grid md:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 text-natural-cream/70">
              <p>{contact?.location || 'Northern Rivers, NSW'}</p>
              <p>{contact?.region || 'Australia'}</p>
              {contact?.email && (
                <a
            ...
            {(social?.facebook || social?.instagram || social?.linkedin) && (
              <div className="flex gap-4 mt-4">
                {social?.facebook && (
                  <a href={social.facebook} target="_blank" rel="noopener noreferrer" className="text-natural-cream/70 hover:text-golden-wattle transition-colors">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                  </a>
                )}
                {social?.instagram && (
                  <a href={social.instagram} target="_blank" rel="noopener noreferrer" className="text-natural-cream/70 hover:text-golden-wattle transition-colors">
                ...
                {social?.linkedin && (
                  <a href={social.linkedin} target="_blank" rel="noopener noreferrer" className="text-natural-cream/70 hover:text-golden-wattle transition-colors">
        ...
        <div className="mt-12 pt-8 border-t border-natural-cream/20 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-natural-cream/50 text-sm">
            &copy; {new Date().getFullYear()} {siteName}. All rights reserved. Australian Innovation Patent.
          </p>
          <p className="text-natural-cream/50 text-sm">
