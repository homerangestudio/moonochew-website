...
import { getPageContent, HomePageContent } from '@/lib/content'
...
export default function Home() {
  const content = getPageContent<HomePageContent>('home')
    ...
    <div className="bg-spring-meadow text-charcoal">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-living-green/10 border border-living-green/30 rounded-full">
                ...
                <span className="text-sm font-semibold text-forest-canopy">{content.hero.badge}</span>
              </div>
              ...
              <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl leading-tight text-deep-forest">
                ...
                <br />
                <span className="text-living-green">{content.hero.headlineHighlight}</span>
              </h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
            <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl mb-4 text-deep-forest">
              {content.problem.headline} <span className="text-clay-earth">{content.problem.headlineHighlight}</span>
            </h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
            {content.problem.cards.map((card, index) => (
              <div key={index} className="bg-white p-6 rounded-2xl shadow-natural border border-parchment text-center">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="order-1 lg:order-2 space-y-6">
              <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-deep-forest">
                {content.solution.headline} <span className="text-living-green">{content.solution.headlineHighlight}</span>
              </h2>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
            {content.benefits.stats.map((stat, index) => (
              <div key={index} className="bg-forest-canopy-dark/50 p-6 rounded-2xl border border-natural-cream/10 text-center">
      ...
      <section className="py-16 md:py-24 bg-gradient-to-br from-living-green via-forest-canopy to-living-green text-natural-cream">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl mb-4">
            {content.cta.headline} <span className="text-golden-wattle">{content.cta.headlineHighlight}</span>
          </h2>
          <p className="text-lg md:text-xl text-natural-cream/80 mb-8 max-w-2xl mx-auto">
            {content.cta.subheadline}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href={content.cta.primaryCtaLink} className="px-8 py-4 bg-golden-wattle text-charcoal font-semibold rounded-xl hover:bg-golden-wattle-light transition-colors shadow-gold">
              {content.cta.primaryCtaText}
            </a>
            <a href={content.cta.secondaryCtaLink} className="px-8 py-4 border-2 border-natural-cream/50 text-natural-cream font-semibold rounded-xl hover:bg-natural-cream/10 transition-colors">
